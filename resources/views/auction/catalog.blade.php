            ...
            <div class="col-sm-6 col-md-4 col-xl-3">
                <div class="card auction-card shadow-sm h-100">
                    <div class="card-img-container"
                        onclick="showFullScreen('{{ asset('img_barang/' . ($foto_utama ?: 'no-image.png')) }}')">
                        <img src="{{ asset('img_barang/' . ($foto_utama ?: 'no-image.png')) }}" alt="{{ $item->nama_barang }}">
                    </div>
                    <div class="card-body d-flex flex-column">
                        ...
                        <div class="mb-3">
                            {{-- Timer menggunakan format ISO agar JavaScript tidak salah hitung [cite: 2026-02-10] --}}
                            <span
                                class="badge bg-light-danger text-danger countdown-timer py-2 px-2 fw-bold w-100 fs-6 font-monospace"
                                data-target="{{ \Carbon\Carbon::parse($item->tgl_status_sold)->toIso8601String() }}"
                                data-style="short">
                        ...
                        <button class="btn btn-primary w-100 py-2 fw-bold shadow-sm" data-bs-toggle="modal"
                            data-bs-target="#modalTawar{{ $item->id_lelang }}">
                                    ...
                                    <div class="col-md-6 d-flex flex-column">
                                        ...
                                        <div class="row mb-3">
                                            <div class="col-6 border-end">
                                                ...
                                                <p class="mb-0 text-primary fw-bold">{{ $item->username }}</p>
                                            </div>
                                            <div class="col-6 ps-3">
                                                <label class="text-muted small fw-bold">TAWARAN TERTINGGI</label>
                                                <p class="mb-0 text-success fw-bold" id="bidder-info-{{ $item->id_lelang }}">
                                                    @if(isset($item->bidder_username))
                                                        {{ $item->bidder_username }}
                                                    @else
                                                        Belum ada
                                                    @endif
                                                </p>
                                        ...
                                        <div class="pt-3 border-top mt-auto">
                                            {{-- FORM ACTION disesuaikan dengan web.php Anda: /auction/bid [cite: 2026-02-10]
                                            --}}
                                            <form action="/auction/bid" method="POST" id="form-bid-{{ $item->id_lelang }}">
                                                @csrf
                                                <input type="hidden" name="id_lelang" value="{{ $item->id_lelang }}">
                                                <div class="d-flex align-items-center gap-2 mb-3">
                                                    <button type="button"
                                                        class="btn btn-outline-danger btn-circle-fixed shadow-sm"
                                                        onclick="adjustBid('{{ $item->id_lelang }}', -1000)"><span>−</span></button>
                                                    <div class="bid-container flex-grow-1 shadow-sm text-center">
                                                        <input type="text" id="displayBid{{ $item->id_lelang }}"
                                                            class="bid-input-display"
                                                            value="{{ number_format($item->harga_awal, 0, ',', '.') }}"
                                                            readonly>
                                                        <input type="hidden" id="inputBid{{ $item->id_lelang }}"
                                                            name="nominal_tawaran" value="{{ $item->harga_awal }}"
                                                            data-min="{{ $item->harga_awal }}">
                                                    </div>
                                                    <button type="button"
                                                ...
                                                <button type="button" class="btn btn-primary btn-lg w-100 fw-bold shadow"
                                                    onclick="confirmBid('{{ $item->id_lelang }}', {{ $is_owner ? 'true' : 'false' }}, '{{ addslashes($item->nama_barang) }}')">
                                                    KIRIM TAWARAN
                                                </button>
                                            </form>
                                        </div>
